...
if ( ! function_exists( 'vive_setup' ) ) :
	...
	function vive_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'vive' ),
		) );
 ...
 */
function vive_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'vive_content_width', 640 );
}
add_action( 'after_setup_theme', 'vive_content_width', 0 );
...
function vive_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'vive' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'vive' ),
...
function vive_scripts() {
	...
	wp_enqueue_script( 'vive-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'vive-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
...
function themebs_enqueue_styles() {
  wp_enqueue_style( 'bootstrap', trailingslashit( get_stylesheet_directory_uri() ) . '/css/bootstrap.css' );
  wp_enqueue_style( 'custom_css',trailingslashit( get_stylesheet_directory_uri() ) . '/css/custom.css' );
}
